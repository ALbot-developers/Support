class faqIndexManager {
	...
	static _formatAsString(faqIndex, faqChannel, categoryId = null) {
		let markdown = '';
		for (const categoryData of Object.values(faqIndex)) {
			if (categoryId && categoryData.id !== categoryId) continue;
	...
	static async format(client, type = String, categoryId = null) {
		try {
			...
			switch (type) {
				case String:
					return this._formatAsString(faqIndex, faqChannel, categoryId);
				case Object: {
					const formatted = await this._formatAsObject(
						faqIndex,
						faqChannel,
						categoryId,
					);
