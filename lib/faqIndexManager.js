class faqIndexManager {
	...
	/**
	 * FAQの目次の見出し（カテゴリ名）だけを抽出する関数
	 * fullIndexGeneratorを使用してFAQ目次を取得し、カテゴリIDとカテゴリ名のマッピングのみを返します。
	 *
	 * @param {Object} client - Discordクライアントオブジェクト
	 * @param {String} [category] - FAQのカテゴリID（オプション）。指定した場合、そのカテゴリのみを返す
	 * @returns {Object|null} - カテゴリIDをキー、カテゴリ名（日本語）を値とするオブジェクト。エラー時はnullを返す
	 *
	 * 戻り値の例:
	 * {
	 *   "bug_bot": "不具合のご報告(BOT)",
	...
	static async indexHeaderGenerator(client, category = null) {
		try {
			...
			for (const catId in faqIndex) {
				headings[catId] = {
					title: faqIndex[catId].categoryNameJP,
					description: faqIndex[catId].description,
				};
			}
			return headings;
		} catch (err) {
